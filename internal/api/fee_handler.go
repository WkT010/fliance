...
func (h *FeeHandler) GetFee(pair string, volume30d float64) (maker, taker float64) {
	...
	if !ok { return 0.001, 0.001 }
	maker, taker = s.DefaultMaker, s.DefaultTaker
	for _, t := range s.Tiers {
		if volume30d >= t.MinVolume && (t.MakerRate < maker || t.TakerRate < taker) { maker, taker = t.MakerRate, t.TakerRate }
	}
	return
}
...
func (h *FeeHandler) GetSchedule(c *gin.Context) {
	...
	for _, s := range h.schedule { schedules = append(schedules, s) }
...
func (h *FeeHandler) GetPairFee(c *gin.Context) {
	...
	s, ok := h.schedule[pair]
	h.mu.RUnlock()
	if !ok { c.JSON(http.StatusNotFound, gin.H{"error": "pair not found"}); return }
...
func (h *FeeHandler) UpdateFee(c *gin.Context) {
	...
	if !ok { s = &FeeSchedule{Pair: req.Pair}; h.schedule[req.Pair] = s }
	if req.DefaultMaker != nil { s.DefaultMaker = *req.DefaultMaker }
	if req.DefaultTaker != nil { s.DefaultTaker = *req.DefaultTaker }
	if req.Tiers != nil { s.Tiers = *req.Tiers }
...
func (h *FeeHandler) CalculateFee(c *gin.Context) {
	...
	if pair == "" || amount == "" { c.JSON(http.StatusBadRequest, gin.H{"error": "pair and amount required"}); return }
	var amt float64
	if _, err := fmt.Sscanf(amount, "%f", &amt); err != nil { c.JSON(http.StatusBadRequest, gin.H{"error": "invalid amount"}); return }
	maker, taker := h.GetFee(pair, 0)
	rate := taker
	if side == "limit" { rate = maker }
	fee := amt * rate
