...
func (c *Consumer) Start() {
	go func() {
		for {
			...
			if err != nil { select { case <-c.done: return; default: log.Printf("consumer err: %v", err); continue } }
			var et, pair string
			for _, h := range m.Headers { switch h.Key { case "event_type": et = string(h.Value); case "pair": pair = string(h.Value) } }
