...
func (s *MatchingServer) PlaceOrder(ctx context.Context, req *pb.PlaceOrderRequest) (*pb.PlaceOrderResponse, error) {
	...
	side := matching.Buy
	if req.Side == pb.Side_SELL { side = matching.Sell }
...
func (s *MatchingServer) GetOrderbook(ctx context.Context, req *pb.GetOrderbookRequest) (*pb.GetOrderbookResponse, error) {
	e, ok := s.engines[req.Pair]
	if !ok { return nil, fmt.Errorf("no engine: %s", req.Pair) }
	depth := e.OrderBook.Depth(int(req.Limit))
	pbDepth := &pb.OrderBookDepth{Pair: depth.Pair, SeqNo: int64(depth.SeqNo)}
	for _, b := range depth.Bids { pbDepth.Bids = append(pbDepth.Bids, &pb.PriceLevel{Price: b.Price.Text('f', 8), Quantity: b.Quantity.Text('f', 8), Count: int64(b.Count)}) }
	for _, a := range depth.Asks { pbDepth.Asks = append(pbDepth.Asks, &pb.PriceLevel{Price: a.Price.Text('f', 8), Quantity: a.Quantity.Text('f', 8), Count: int64(a.Count)}) }
...
func (s *StreamServer) StreamOrderbook(req *pb.StreamOrderbookRequest, stream pb.StreamService_StreamOrderbookServer) error {
	...
	for {
		select {
		case <-ticker.C:
			depth := e.OrderBook.Depth(100)
			if depth.SeqNo == last { continue }; last = depth.SeqNo
			pbDepth := &pb.OrderBookDepth{Pair: depth.Pair, SeqNo: int64(depth.SeqNo)}
			for _, b := range depth.Bids { pbDepth.Bids = append(pbDepth.Bids, &pb.PriceLevel{Price: b.Price.Text('f', 8), Quantity: b.Quantity.Text('f', 8), Count: int64(b.Count)}) }
			for _, a := range depth.Asks { pbDepth.Asks = append(pbDepth.Asks, &pb.PriceLevel{Price: a.Price.Text('f', 8), Quantity: a.Quantity.Text('f', 8), Count: int64(a.Count)}) }
			stream.Send(&pb.StreamOrderbookResponse{Depth: pbDepth})
		case <-stream.Context().Done(): return stream.Context().Err()
		}
	}
}
