...
func TestSimpleMatch(t *testing.T) {
	...
	select {
	case tr := <-e.Trades:
		if tr.Price.Cmp(big.NewFloat(50000)) != 0 {
			t.Errorf("bad price: %s", tr.Price.String())
		}
		if tr.Quantity.Cmp(big.NewFloat(1.0)) != 0 {
			t.Errorf("bad qty: %s", tr.Quantity.String())
		}
	default:
...
func TestPricePriority(t *testing.T) {
	...
	if trades[0].Price.Cmp(big.NewFloat(50000)) != 0 {
		t.Error("expected first fill at 50000 (lowest ask)")
	}
}
...
func TestFOKAccepted(t *testing.T) {
	...
	e.SubmitOrder(fok)
	if fok.Status == Rejected {
		t.Error("FOK should NOT reject when enough liquidity")
	}
}
...
func TestFOKRejected(t *testing.T) {
	...
	e.SubmitOrder(fok)
	if fok.Status != Rejected {
		t.Error("FOK should reject when insufficient liquidity")
	}
}
...
func TestIOCPartialFill(t *testing.T) {
	...
	if ioc.Status != Cancelled {
		t.Error("IOC should cancel remaining qty")
	}
}
...
func benchDrainTrades(ch <-chan *Trade) []*Trade {
	var trades []*Trade
	for {
		select {
		case t := <-ch:
