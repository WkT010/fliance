...
export class NexaSocket {
  ...
  connect(pair: string) {
    ...
    this.ws.onclose = () => {
      if (!this.manualClose) {
        this.reconnectTimer = setTimeout(() => this.pair && this.connect(this.pair), 3000);
      }
    };
  ...
  private subscribe(channel: string, pairs: string[]) {
    this.send({ type: 'subscribe', channel, pairs });
  }
